...
try
    ...
    mys = max(ye);
    mid = round(mean(find(ye==mys)));    
    hold(axVSI,'on');
    hp = plot(axVSI,xi,ye,'--r',xi(mid),mys,'ob');
    title(axVSI,['Maximum : <ID-' num2str(mid) ',' num2str(xi(mid)) ' nm >'])
    hold(axVSI,'off');
    axis(axVSI,[startID*dh endID*dh 0 255])
    drawnow
    save([folder_name '/vsi_data/analysis_curves.mat'],'startID','endID','dh','I','xi','ye','mid','mys');            
catch ex
    hmsg = msgbox(ex.message,'Peak detection test error','warn','modal');
    frames = java.awt.Frame.getFrames();
    frames(end).setAlwaysOnTop(1);
    uiwait(hmsg);
end